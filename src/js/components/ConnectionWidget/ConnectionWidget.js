...
export default class ConnectionWidget extends Component {
    constructor(props) {
        super(props);

        this.state = {
            provider: (props.provider) ? props.provider : 'wordpress',
            postStatus: (props.postStatus) ? props.postStatus : null,
    ...
    getProvider() {
        switch (this.state.provider) {
            case 'scatter':
                return <svg viewBox="0 0 33 33" xmlns="http://www.w3.org/2000/svg">
            ...
            default:
                return <img src={wordproofData.images.wordpress} alt="wordpress logo"/>;
        }
    }
    ...
    render() {
        return (
            <div className="wordproof-connection-widget" data-wordproof-connection-status={this.state.status}>
          ...
          {this.getProvider()}
            ...
            <circle cx="105" cy="15" r="15">
              ...
              />
                ...
                <span
                    className="wordproof-connection-widget__status">{(this.state.text) ? this.state.text.message : this.getText().message}
                    <strong>{(this.state.text) ? this.state.text.messageStrong : this.getText().messageStrong}</strong></span>
            </div>
        );
    }
}
